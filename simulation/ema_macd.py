...
def simulate_params(pair, df, df_m5,  slow, fast, signal, ema, time_d):
    ...
    # Add columns to results to filter out for analyzing later
...
def run_pair(pair):
    ...
    # Gets hour and 5 minute candles
    df, df_m5 = load_data(pair, time_d=time_d)
    
    results = [] # Holds dataframes of results summary for a given strategy
    trades = [] # Holds individual trades

    print("\n--> Running", pair)

    # For each MACD-EMA pair loop through different parameters
    for slow in [26,52]:
        for fast in [12,18]:
            if slow <= fast:
                continue
...
'''
Sets up currency pairs and runs MACD-EMA guru test on them
'''
